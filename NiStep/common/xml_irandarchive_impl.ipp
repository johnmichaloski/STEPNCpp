...
xml_irandarchive_impl<Archive>::load(std::wstring &ws){
    std::string s;
...
template<class Archive>
BOOST_ARCHIVE_DECL(void)
xml_irandarchive_impl<Archive>::load(wchar_t * ws){
    std::string s;
    bool result = gimpl->parse_string(is, s);
    if(! result)
        boost::serialization::throw_exception(
...
template<class Archive>
BOOST_ARCHIVE_DECL(void)
xml_irandarchive_impl<Archive>::load(std::string &s){
    bool result = gimpl->parse_string(is, s);
    if(! result)
        boost::serialization::throw_exception(
            xml_archive_exception(xml_archive_exception::xml_archive_parsing_error)
...
template<class Archive>
BOOST_ARCHIVE_DECL(void)
xml_irandarchive_impl<Archive>::load(char * s){
    std::string tstring;
...
template<class Archive>
BOOST_ARCHIVE_DECL(void)
xml_irandarchive_impl<Archive>::load_override(class_name_type & t, int)
{
#if 0
...
template<class Archive>
BOOST_ARCHIVE_DECL(void)
xml_irandarchive_impl<Archive>::init(){
 //   gimpl->init(is);
...
xml_irandarchive_impl<Archive>::xml_irandarchive_impl(
    unsigned int flags
) :
