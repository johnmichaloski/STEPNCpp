...
{	
	CString str;
	str.Format(_T("<CENTER><form><input type=button value=\"%s\" onClick=\"printpage()\"></form></CENTER>"), button);
	return str;
}
inline CString IncludeJavaScript(CString name)
{	
	return StrFormat(_T("<script src=\"%s\" type=\"text/javascript\"></script>"), name);
}
...
public:
	...
	CString TableFormat()
	{
		return StrFormat(_T("<table bgcolor=\"%s\" width=\"%s\"  border=\"%s\" cellpadding=\"%s\" cellspacing=\"%s\" style=\"%s\">\n"),
			tablebgcolor,  tablewidth, tableborder, cellpadding, cellspacing,tablestyle);
	}
	CString TableHeaderRowFormat()
	{
		return StrFormat(_T("<tr align=\"%s\" bgcolor=\"#%06X\" style=\"%s\">\n"),  headeralign, headercolor,  headerstyle); 
	} 
	...
	CString ToHtmlTable()
	{
		...
		{
			tmp.AppendFormat(_T("<TD>%s</TD>\n"),  m_strTableHeader[i]);
		}
		tmp+=_T("</TR>\n");

		for(i=0; i< m_strTableData.size(); i++)
		{
			CString bstrRowStyle = (m_strRowStyle.size()>i)?  m_strColumnStyle[j] : _T("");
			tmp.AppendFormat(_T("<TR %s>"),  bstrRowStyle);
			for(j=0; j< m_strTableData[i].size(); j++)
			{
				CString bstrColStyle= (m_strColumnStyle.size()>j)?  m_strColumnStyle[j] : _T("");
				tmp.AppendFormat(_T("<TD %s >"),  bstrColStyle);
