...
class StepNCFacing : public   IWorkCycle
{
public:
	StepNCFacing(IWorkCycles * owner) ;
	virtual ~StepNCFacing(){}
	...
	END_STEP_MAP(StepNCFacing)

	BEGIN_VERIFY_PARAMETER_MAP(StepNCFacing)
	...
	END_VERIFY_PARAMETER_MAP(StepNCFacing)
	IStepNodePtr _project;
};
FEATURE_DECLARE_SMARTPOINTER(StepNCFacing);

inline StepNCFacing::StepNCFacing(IWorkCycles * owner) :   IWorkCycle(owner)
{ 
	_parent=owner;
	InitStepItems(&items);
	InitTests(this);
...
inline HRESULT StepNCFacing::DeriveParameters(IStepNodePtr pExecutable, IStepNodePtr pFeature, IStepNodePtr pOpItem)
{
...
inline HRESULT StepNCFacing::Create(IStepNodePtr project)
{
	if(project==NULL)
		return E_INVALIDARG;
...
inline HRESULT StepNCFacing::LoadIntoStep() 
{ 
