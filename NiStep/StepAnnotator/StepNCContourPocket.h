...
class StepNCContourPocket : public   IWorkCycle
{
public:
	StepNCContourPocket(IWorkCycles * owner=NULL) ;
	virtual ~StepNCContourPocket(){}
	...
	virtual HRESULT Create(IStepNodePtr, CString opname);
	virtual void InitValues(CCannedCycleItems *pItems){}
	BEGIN_STEP_MAP(StepNCContourPocket)
		...
		ENTRY(_T("Contour"),				Cycle::Feature,		Cycle::Polyline,	_T("feature_boundary{general_closed_profile}/closed_profile_shape{composite_curve}")) 
	...
	END_STEP_MAP(StepNCContourPocket)

	BEGIN_VERIFY_PARAMETER_MAP(StepNCContourPocket)
	...
	END_VERIFY_PARAMETER_MAP(StepNCContourPocket)
	IStepNodePtr _project;
};
FEATURE_DECLARE_SMARTPOINTER(StepNCContourPocket);

inline StepNCContourPocket::StepNCContourPocket(IWorkCycles * owner) :   IWorkCycle(owner)
{ 
	_parent=owner;
	InitStepItems(&items);
	InitTests(this);
...
inline HRESULT StepNCContourPocket::DeriveParameters(IStepNodePtr pExecutable, IStepNodePtr pFeature, IStepNodePtr pOpItem)
{
...
inline HRESULT StepNCContourPocket::Create(IStepNodePtr project, CString opname)
{
	if(project==NULL)
		return E_INVALIDARG;

	if(project->GetClassname() != _bstr_t(L"project"))
		throw std::exception("Bad Step project instance");
...
inline HRESULT StepNCContourPocket::LoadIntoStep() 
{ 
	...
	// FIXME: depth need to be turned into axis2d with proper orientation
	double depth = items->GetValue<double>(_T("AbsDepth"), -999.0);
	double securityPlane = items->GetValue<double>(_T("SecurityPlane"), -999.0);
	double startAbscissa = items->GetValue<double>(_T("StartAbscissa"), -999.0);
	double startOrdinate = items->GetValue<double>(_T("StartOrdinate"), -999.0);
	double referencePlane = items->GetValue<double>(_T("ReferencePlane"), -999.0);
	// Location orientation

	addons.DefineAxis2_placement_3d(_pFeature, _T("depth/position"), _T("depth"), Vector(0,0, depth).ToString(_T(",")));
	addons.DefineAxis2_placement_3d(_pExecutable, _T("its_secplane/position"), _T("security plane"), Vector(0,0, securityPlane).ToString(_T(",")));
	addons.DefineAxis2_placement_3d(_pExecutable, _T("its_feature/feature_placement"), _T("feature pos orientation"), Vector(startAbscissa, startOrdinate, referencePlane).ToString(_T(",")));

	// FIXME: location need to be turned into axis2d
	...
	{
		long n = items->GetValue<long>(_T("CycleType"), 1);
		if(n==1)
		{
