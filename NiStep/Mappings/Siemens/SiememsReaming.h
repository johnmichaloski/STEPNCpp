...
////////////////////////////////////////////////////////////////////////
class SiemensReaming : public   IWorkCycle
{
public:
	SiemensReaming(IWorkCycles * owner);
	...
	BEGIN_STEP_MAP(SiemensReaming)
#if 0
		ENTRYSET(_T("Type"),			Cycle::Compute,		Cycle::String,		_T(""), _T("Reaming"))
	...
	END_STEP_MAP(SiemensReaming)

	BEGIN_OUTPUT_PARAMETER_MAP(SiemensReaming)
		...
		STEPNC_OUTPUT_ENTRY(_T("Both"), _T("CYCLE85("), _T(""), _T(""))
		...
		STEPNC_OUTPUT_ENTRY(_T("Both"), _T("%.3f "), _T("RetractionFeedrate"), _T(""))
		STEPNC_OUTPUT_ENTRY(_T("Both"), _T(")\n"), _T(""), _T(""))
	END_OUTPUT_PARAMETER_MAP(SiemensReaming)

	BEGIN_VERIFY_PARAMETER_MAP(SiemensReaming)
	END_VERIFY_PARAMETER_MAP(SiemensReaming)

};
FEATURE_DECLARE_SMARTPOINTER(SiemensReaming);


SiemensReaming::SiemensReaming(IWorkCycles * owner):  IWorkCycle(owner)
{
	_sName=_T("Reaming"); 
	_parent=owner;
	InitStepItems(&items);
...
HRESULT SiemensReaming::DeriveParameters(IStepNodePtr pExecutable, IStepNodePtr pFeature, IStepNodePtr pOpItem)
{
	IWorkCycle::DeriveParameters(pExecutable, pFeature, pOpItem);

	CCannedCycleItems * items = GetStepMap();  // initialize output for now...	

	items->SetVariable(_T("CycleType"),  2) ;
