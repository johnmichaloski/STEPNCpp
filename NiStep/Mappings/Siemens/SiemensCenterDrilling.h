...
class SiemensCenterDrilling : public   IWorkCycle
{
public:
	SiemensCenterDrilling(IWorkCycles * owner) ;
	virtual ~SiemensCenterDrilling(){}
	virtual HRESULT DeriveParameters(IStepNodePtr pExecutable, IStepNodePtr pFeature, IStepNodePtr pOpItem);

	BEGIN_STEP_MAP(SiemensCenterDrilling)
#if 0
		ENTRYSET(_T("Type"),			Cycle::Compute,		Cycle::String,		_T(""), _T("CenterDrilling"))
		...
		ENTRY(_T("CycleType"),			Cycle::Compute,		Cycle::Long,		_T("CycleType = 1.0 + Find(Operation, \")finish\_T(")")) 
	...
	END_STEP_MAP(SiemensCenterDrilling)

	BEGIN_OUTPUT_PARAMETER_MAP(SiemensCenterDrilling)
		// Approach
		...
		STEPNC_OUTPUT_ENTRY(_T("Both"), _T("CYCLE81("), _T(""), _T(""))
	...
	END_OUTPUT_PARAMETER_MAP(SiemensCenterDrilling)

	BEGIN_VERIFY_PARAMETER_MAP(SiemensCenterDrilling)
	...
	END_VERIFY_PARAMETER_MAP(SiemensCenterDrilling)

};
FEATURE_DECLARE_SMARTPOINTER(SiemensCenterDrilling);

SiemensCenterDrilling::SiemensCenterDrilling(IWorkCycles * owner) :   IWorkCycle(owner)
{ 
	_parent=owner;
	InitStepItems(&items);
	InitOutputs(this);
...
HRESULT SiemensCenterDrilling::DeriveParameters(IStepNodePtr pExecutable, IStepNodePtr pFeature, IStepNodePtr pOpItem)
{
