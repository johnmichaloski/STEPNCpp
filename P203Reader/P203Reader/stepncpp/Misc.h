...
{
public:
	int FileOperation(CString fromfile, CString folder, UINT fileop,  FILEOP_FLAGS flags  )
	{
		...
		strncpy(FromFile,fromfile, MAX_PATH-1); 
	...
	int CopyFileToFolder(CString fromfile, CString tofile, FILEOP_FLAGS flags = FOF_NOCONFIRMATION | FOF_SILENT )
	{
		return FileOperation(fromfile,  tofile, FO_COPY, flags  );
	}
	int RenameFile(CString fromfile, CString tofile, FILEOP_FLAGS flags = FOF_NOCONFIRMATION | FOF_SILENT )
	{
		return FileOperation(fromfile,  tofile, FO_RENAME, flags  );
	}
	...
	{ 
		...
		//add an extra NULL to the end of From 
		strncpy(FromFile,From, MAX_PATH-1); 
		FromFile[strlen(FromFile)+1]=NULL; 
