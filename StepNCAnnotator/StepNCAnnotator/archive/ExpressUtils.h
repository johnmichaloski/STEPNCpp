...
{
	T step = (T::element_type *) (IStepNode *) gClassGenerators.CreateClass(ExpDemangle(_sListClassname),"").get();
	if(step==NULL)
		return step; 
...
inline 	bool EListT<T>::IsSimpleListType(CString classname)
{
	...
	T step = (T::element_type *) (IStepNode *) gClassGenerators.CreateClass(ExpDemangle(classname),"").get();
	if(step==NULL)
	{
		//DebugBreak();
		return false;
