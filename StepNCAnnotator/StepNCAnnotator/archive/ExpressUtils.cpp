...
{
	IStepNodePtr pDerivedClass=  gClassGenerators.CreateClass(ExpDemangle(classname), "");
...
void CloneCVarDef(CVarDefPtr  fromvardef, CVarDefPtr  tovardef)
{
	...
	else if(fromvardef->Type() == 	bag_ )
	{
		...
		for (int i = 0; i<fromlist->size(); i++)
		{
			...
			IStepNodePtr tostep =  (IStepNode *) gClassGenerators.CreateClass(szClassname,"").get();
