...
// trim from start
inline std::string &TrimLeft(std::string &str, std::string tokens=" \t\r\n") 
{

	size_t startpos = str.find_first_not_of(tokens); 
...
// trim from end
inline std::string &TrimRight(std::string &str, std::string tokens=" \t\r\n") 
{
 		size_t endpos = str.find_last_not_of(tokens); 
...
inline bool StartsWith(std::string &fullstring, std::string &beginning)
{
...
template<typename T>
inline T ConvertFromString(std::string data, T defaultval)
{
	T result;
