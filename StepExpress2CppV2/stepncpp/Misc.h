...
public:
	...
	CString ToString(CString separator=_T(" ")) { 
		CString tmp; for(UINT i=0; i< size(); i++) 
			tmp.AppendFormat("%s%s", at(i), separator);  
		return tmp; 
	}
...
inline CString NewLoggingFileName(CString filename, CString szLoggingDirectory, CString szFormat=_T("%A, %B %d %Y %H_%M_%S"))
{
	COleDateTime now = COleDateTime::GetCurrentTime ();
	CString szDate = now.Format( szFormat );
	//time_t ltime;
	//time( &ltime );
	//filename.AppendFormat("%s", ctime( &ltime ) );
	filename+=szDate;
	filename.Replace(":","_");
	filename.Replace("\n","");
	if(szLoggingDirectory.Right(szLoggingDirectory.GetLength()-2)!= _T("\\"))
		szLoggingDirectory+="\\";

	return szLoggingDirectory+filename;
}
