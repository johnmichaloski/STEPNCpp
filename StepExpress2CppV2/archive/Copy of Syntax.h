...
typedef const char * IteratorT; //Obviously, this should be whatever iterator you're using for your spirit parser
	...
	public boost::spirit::grammar<CSyntax>
{
	...
	public:
		...
		{   
			...
			STRING_LITERAL	 = lexeme_d[chlit<>('\"') >> *( strlit<>("\\\"") | anychar_p - chlit<>('\"') ) >> chlit<>('\"')]; //[&write]
			...
			cmd = voidcommand >> chseq_p("()") 
				...
				| str_p("AlignPart")  >> '(' >>  list_p( real_p , ',') >> ')'  
				| str_p("AlignTool") >> '(' >>  list_p( real_p , ',') >> ')'
				| str_p("CenterPart")  >> '(' >>  real_p >> ',' >>  real_p >> ',' >>  real_p >> ',' >>  real_p  >> ')'

				| str_p("EnumAllProp")  >> '(' >> enum_prop_list >> ')'
				| str_p("EnumProp")  >> '(' >> enum_prop_list >> ')'

				| str_p("GoTo") >> '(' >> list_p( gotocmd_item , ',' ) >> ')'
				| str_p("Get") >> '(' >> get_list >> ')' 
				| str_p("GetCsyTransformation") >>  '(' >> csytransformations >> ')'

				| str_p("GetErrorInfo") >> '(' >>  int_p >>  ')'
				| str_p("GetProp") >> '(' >>  getprop_list >>  ')'

				| str_p("PtMeas") >> '(' >> ptmeas_list >> ')'
				| str_p("PtMeasSelfCenterLocked") >> '(' >> list_p(ptmeasselfcenter_props >>  chseq_p("()"), ',')  >> ')'

				| str_p("OnPtMeasReport") >>  '(' >> on_ptmeas_report_list >> ')'
				| str_p("OnScanReport") >>  '(' >> on_ptmeas_report_list >> ')'



				| str_p("LockAxis") >> '(' >> lock_axis_prop_get_list >>')'
				| str_p("LockPosition") >> '(' >>lock_position_prop_get_list >> ')'
				| str_p("ScanOnCircleHint") >> '('>> real_p /* displacement*/ >> ','>> real_p /*form*/ >> ')'
				| str_p("ScanOnLineHint") >> '('>> real_p /* angle*/ >> ','>> real_p /*form*/ >> ')'
				| str_p("ScanOnLineHint") >> '('>> real_p /* deviation*/ >> ','>> real_p /*minradiusofcurvature*/ >> ')'
				| str_p("ScanUnknownHint") >> '('>> real_p /* minradiusofcurvature*/ >> ')'

				// this will catch good and bad scans
				| str_p("ScanOnCircle") >> '('>> list_p(real_p, ',') >> ')'
				| str_p("ScanOnLine") >> '('>> list_p(real_p, ',') >> ')'
				| str_p("ScanInPlaneEndIsSphere") >> '('>> list_p(real_p, ',') >> ')'
				| str_p("ScanInPlaneEndIsPlane") >> '('>> list_p(real_p, ',') >> ')'
				| str_p("ScanInPlaneEndIsCyl") >> '('>> list_p(real_p, ',') >> ')'
				| str_p("ScanInCylEndIsSphere") >> '('>> list_p(real_p, ',') >> ')'
				| str_p("ScanInCylEndIsPlane") >> '('>> list_p(real_p, ',') >> ')'
				| str_p("ScanOnCurveDensity") >>  '(' >>  list_p(
				str_p("Dis") | str_p("Angle") |	str_p("AngleBaseLength") |(str_p("AtNominals") )>> '(' >>real_p>>  ')' , ',') >> ')'

				| str_p("ScanOnCurve") >> '(' >> str_p("Closed") >> '(' >> int_p  >> chseq_p("),") >>
				...
				str_p("Data") >>  '(' >>
				list_p(real_p, ',') >> ')'
				| str_p("SetCoordSystem")>>'(' >> coordsystem >> ')' 
				| str_p("SaveNamedCsyTransformation") >> '(' >> dblquoted_string >> ',' >> repeat_p(6)[',' >> real_p ] >>')'
				| str_p("SetCsyTransformation") >> '(' >> csytransformations >>  repeat_p(6)[',' >> real_p ] >>')'
				| str_p("SetProp") >> '(' >> setprop_list >> ')'

				| str_p("StopDaemon") >> '(' >> event_num >> ')' 
				| str_p("TiltCenterPart") >> '(' >> real_p >> repeat_p(6)[',' >> real_p ] >>')'
				| str_p("TiltPart") >> '(' >> real_p >> repeat_p(3)[',' >> real_p ] >>')'
				;
			eventcmd =
